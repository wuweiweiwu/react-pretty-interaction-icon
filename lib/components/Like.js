...
class Like extends Component {
  ...
  componentDidMount() {
    ...
    const iconTravel = new mojs.Tween({
			...
			onUpdate: function(progress) {
				...
				likeIcon.style.WebkitTransform = likeIcon.style.transform = `translate3d(${-50*(1-elasticOutProgress)}%,0,0)`;
			}
		})
  ...
  render() {
    ...
    const labelPositionTop = parseInt(sizes.square[size].replace('px', '')) * labelPositionMultipliers[labelPosition].top;
    const labelPositionLeft = parseInt(sizes.square[size].replace('px', '')) * labelPositionMultipliers[labelPosition].left;
