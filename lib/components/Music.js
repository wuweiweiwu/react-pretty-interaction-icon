...
class Music extends Component {
  ...
  static propTypes = {
    ...
    noteColors: function(props, propName, componentName) {
      ...
      return noteColors.length <= 4 && noteColors.length >= 0? null : new Error("Length must be between 0 and 4");
    },
  }
  ...
  componentDidMount() {
    const { hasRock, noteColors } = this.props;
    ...
    const note1 = new mojs.ShapeSwirl({
      ...note_opts_two,
      fill: noteColors[0] ? noteColors[0] : randomColor(),
      opacity: { 1: 0 },
      swirlSize: 15,
      swirlFrequency: 20
    });
    ...
    const note2 = new mojs.ShapeSwirl({
      ...
      fill: noteColors[1] ? noteColors[1] : randomColor(),
      y: {
        20: -30
      },
      opacity: { 1: 0 },
    ...
    const note3 = new mojs.ShapeSwirl({
      ...note_opts_one,
      fill: noteColors[2] ? noteColors[2] : randomColor(),
      y: {
        20: -70
      },
      opacity: { 1: 0 },
    ...
    const note4 = new mojs.ShapeSwirl({
      ...
      fill: noteColors[3] ? noteColors[3] : randomColor(),
      y: {
        10: -40
      },
      opacity: { 1: 0 },
    ...
    const rockingEasing = mojs.easing.path('M0,351S16.7,59.7,64.7,60.3c0,0,25-11,26.3,139.7,1.3,144.3,31.3,129.3,31.3,129.3S159,339,159,200c0-147,37-139.9,37-139.9S226.4,45.7,223.7,200c-2.7,151.2,24.7,128,24.7,128s36.9,10.5,38.7-128c2-153,39.7-140.4,39.7-140.4s36.3-9.9,41,186.1c0,0,.7,110.3,32.3,105.3');
    const jumpEasing = mojs.easing.path('M0,400,77.7,43,117,239,143,77s16,264,42.7,251.3,11.7-379,37.7-315S235,303,257,235.7,281,61.7,285.7,97,305,262.3,311,209.3s25.3-119,34-46.3,55,237,55,237');
    const iconRock = new mojs.Tween({
      ...
      onUpdate: function (progress) {
        ...
        musicIcon.style.transform = `translateX(${rockingProgress}px) translateY(${0.25*jumpProgress}px)  rotate(${rockingProgress}deg)`;
      }
    });
    ...
    if (hasRock) { animationTimeline.add(iconRock); }
